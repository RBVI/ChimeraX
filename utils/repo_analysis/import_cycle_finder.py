...
def find_cycles(arg):
    ...
    imports_from_sources = subprocess.run(
        ["grep", "\\/src\\/"], input=imports.stdout, capture_output=True
    )
    chimerax_imports = subprocess.run(
        ["grep", "chimerax"], input=imports_from_sources.stdout, capture_output=True
    )
    ...
    if arg == "out_degree":
        nodes_by_out_degree = sorted(
            [(node, bundle_graph.out_degree(node)) for node in bundle_graph],
            key=lambda x: x[1],
        )
        print(nodes_by_out_degree)
    elif arg == "in_degree":
        nodes_by_in_degree = sorted(
            [(node, bundle_graph.in_degree(node)) for node in bundle_graph],
            key=lambda x: x[1],
        )
