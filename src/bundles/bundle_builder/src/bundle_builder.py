...
def _dylib_archs(path):
    ...
    if magic in (0xCAFEBABE, 0xCAFEBABF):  # fat binary: big-endian header
        ...
        return archs
    # thin Mach-O: magic is little-endian on all archs we support
...
def _find_openmp_mac():
    ...
    for path in candidates:
        ...
        if archs and arch not in archs:
            print(
                "bundle_builder: ignoring %s (built for %s, need %s)"
...
class BundleBuilder:
    ...
    def _copy_openmp_lib(self):
        ...
        if not omp_lib:
            self.logger.warning(
                "libomp for this architecture was not found; modules declaring "
                "usesOpenMP will be built single-threaded. Install libomp "
                "(brew install libomp) or set CHIMERAX_LIBOMP_ROOT."
            )
            return
        import shutil
        import subprocess
