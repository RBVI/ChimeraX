...
def mutation_scores_statistics(session, score_name = None, scores_name = None,
                               subtract_fit = None, type = 'synonymous'):
    ...
    message = f'Column {score_name}, {len(values)} {type} mutations, mean = {"%.3g"%mean}, standard deviation = {"%.3g"%std}, mean -/+ 2*SD = {"%.3g"%(mean-2*std)} to {"%.3g"%(mean+2*std)}'
