...
from ..data_model import AvailableDBs, AvailableMatrices, CurrentDBVersions
...
class BlastProteinTool(ToolInstance):
    ...
    def _build_ui(self):
        ...
        # Row 1
        self.menu_widgets["matrices"] = BlastProteinFormWidget(
            "Matrix", QComboBox, self.input_container_row1
        )
        self.menu_widgets["cutoff"] = BlastProteinFormWidget(
            "Cutoff 1e", QSpinBox, self.input_container_row1
        )
        self.menu_widgets["sequences"] = BlastProteinFormWidget(
            "# Sequences", QSpinBox, self.input_container_row1
        )

        self.menu_widgets["cutoff"].input_widget.setRange(-100, 100)
        self.menu_widgets["cutoff"].input_widget.setButtonSymbols(
            QAbstractSpinBox.NoButtons
        )
        self.menu_widgets["sequences"].input_widget.setRange(1, 5000)
        self.menu_widgets["sequences"].input_widget.setButtonSymbols(
            QAbstractSpinBox.NoButtons
        )

        # Row 2
        self.menu_widgets["chain"] = BlastProteinFormWidget(
            "Query", input_widget=None, parent=self.input_container_row2
        )
        self.menu_widgets["_chain_button"] = ChainMenuButton(
            self.session,
            no_value_button_text="No chain chosen",
            parent=self.menu_widgets["chain"],
        ...
        self.menu_widgets["database"] = BlastProteinFormWidget(
            "Database", QComboBox, self.input_container_row2
        )
        self.menu_widgets["version"] = BlastProteinFormWidget(
            "Version", QSpinBox, self.input_container_row2
        )
        self.menu_widgets["version"].input_widget.setButtonSymbols(
            QAbstractSpinBox.NoButtons
        )
        ...
        # Row 4
        self.menu_widgets["seq_input"] = QPlainTextEdit(
            parent=self.input_container_row4
        )
        ...
        for widget in ["help", "apply", "reset", "close", "ok"]:
            self.menu_widgets[widget].setSizePolicy(
        ...
        self.menu_widgets["chain"].input_widget.value_changed.connect(
            self._on_chain_menu_changed
        )
        self.menu_widgets["sequences"].input_widget.valueChanged.connect(
            self._on_num_sequences_changed
        )
        self.menu_widgets["cutoff"].input_widget.valueChanged.connect(
            self._on_cutoff_value_changed
        )
        self.menu_widgets["database"].input_widget.currentIndexChanged.connect(
            self._on_database_changed
        )

        self.menu_widgets["help"].clicked.connect(
            lambda *, run=run, ses=self.session: run(ses, " ".join(["open", self.help]))
        )
        ...
        if self._uniprot_id:
            ...
            pass
        self.menu_widgets["database"].input_widget.setCurrentIndex(
            AvailableDBs.index(self._current_database)
        )
        self.menu_widgets["sequences"].input_widget.setValue(self._num_sequences)
        self.menu_widgets["matrices"].input_widget.setCurrentIndex(
            AvailableMatrices.index(self._current_matrix)
        )
        ...
        if self._current_database == "esmfold":
            self.menu_widgets["version"].input_widget.setRange(
                0, CurrentDBVersions[self._current_database]
            )
        ...
        self.menu_widgets["version"].input_widget.valueChanged.connect(
    ...
    def _reset_options(self) -> None:
        ...
        self.menu_widgets["database"].input_widget.setCurrentIndex(
            AvailableDBs.index("pdb")
        )
        self.menu_widgets["sequences"].input_widget.setValue(100)
        self.menu_widgets["matrices"].input_widget.setCurrentIndex(
            AvailableMatrices.index("BLOSUM62")
        )
            ...
            try:
                ...
                if len(self.session.models) == 0:
                    err = (
                        err
                        + " "
                        + "Please open a model and select a chain, or input sequences or a UniProt ID."
                    )
                raise UserError(err)
        cmd_text = [
            "blastprotein"
            ...
            blast_input,
            "database",
            self.menu_widgets["database"].input_widget.currentText(),
            "cutoff",
            "".join(["1e", str(self._cutoff)]),
            "matrix",
            self.menu_widgets["matrices"].input_widget.currentText(),
            "maxSeqs",
            str(self._num_sequences),
            "version",
            str(self._version),
            "name",
            make_instance_name(),
    ...
    def _on_database_changed(self, _) -> None:
        self._current_database = self.menu_widgets[
            "database"
        ...
        if self._current_database == "alphafold":
            ...
            self.menu_widgets["version"].input_widget.hide()
            self.menu_widgets["version"].label.hide()
        self.menu_widgets["version"].input_widget.setValue(
            CurrentDBVersions[self._current_database]
        )
