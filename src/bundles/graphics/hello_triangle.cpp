...
public:
  ...
  void clearToBlack() {
    // Simplified version just to test clearing
    uint32_t imageIndex;
    ...
    if (result != VK_SUCCESS) {
      std::cout << "Failed to acquire swapchain image: " << result << std::endl;
      return;
    }

    // Record a command buffer that just clears
    ...
    vkCmdBeginRenderPass(commandBuffers[0], &renderPassInfo,
