...
const std::vector<Vertex> left_vertices = {{{0.0f, -0.5f}, {1.0f, 0.0f, 0.0f}}, {{0.5f, 0.5f}, {0.0f, 1.0f, 0.0f}}, {{-0.5f, 0.5f}, {0.0f, 0.0f, 1.0f}}};
const std::vector<Vertex> right_vertices = {{{0.0f, -0.5f}, {0.0f, 1.0f, 0.0f}}, {{0.5f, 0.5f}, {0.0f, 0.0f, 1.0f}}, {{-0.5f, 0.5f}, {1.0f, 0.0f, 0.0f}}};
...
public:
	...
	void setWindowSystemType(SurfaceBackend backend_type) {
		m_windowing_backend = backend_type;
	}
	...
	void createSurface(VkInstance &instance, VkSurfaceKHR &surface, uintptr_t window_id, uintptr_t display_connection = 0) {
#ifdef __APPLE__
		...
		if (result != VK_SUCCESS)
			throw std::runtime_error("failed to create window surface!");
#else // linux
		if (m_windowing_backend == SurfaceBackend::Wayland) {
			VkWaylandSurfaceCreateInfoKHR surfaceInfo{};
			surfaceInfo.sType = VK_STRUCTURE_TYPE_WAYLAND_SURFACE_CREATE_INFO_KHR;
			surfaceInfo.display = (wl_display *)display_connection;
			surfaceInfo.surface = (wl_surface *)window_id;
			auto result = vkCreateWaylandSurfaceKHR(instance, &surfaceInfo, nullptr, &surface);
			if (result != VK_SUCCESS)
				throw std::runtime_error("failed to create window surface!");
		}
		if (m_windowing_backend == SurfaceBackend::Xcb) {
			VkXcbSurfaceCreateInfoKHR surfaceInfo{};
			surfaceInfo.sType = VK_STRUCTURE_TYPE_XCB_SURFACE_CREATE_INFO_KHR;
			surfaceInfo.connection = (xcb_connection_t *)display_connection;
			surfaceInfo.window = window_id;
			auto result = vkCreateXcbSurfaceKHR(instance, &surfaceInfo, nullptr, &surface);
			if (result != VK_SUCCESS)
				throw std::runtime_error("failed to create window surface!");
		}
#endif
...
private:
	...
	/* cast as needed for Xcb, Xlib, or Wayland */
	uintptr_t display_connection;
	bool m_offscreen;

	SurfaceBackend m_windowing_backend;
	...
	std::unordered_map<uint32_t, VkSurfaceKHR> surfaces;

	void set_display_connection(uintptr_t connection) {
	}

	void add_surface(uint32_t win_id) {
		VkSurfaceKHR surface;
		auto instance = m_context->getInstance();
		surfaces[win_id] = surface;
		createSurface(instance, surface, win_id);
	}

	void remove_surface(uint32_t win_id) {
		auto instance = m_context->getInstance();
		vkDestroySurfaceKHR(instance, surfaces[win_id], nullptr);
		surfaces.erase(win_id);
	}
	...
	void createVertexBuffer() {
		...
		bufferInfo.size = sizeof(left_vertices[0]) * left_vertices.size();
		...
		memcpy(data, left_vertices.data(), (size_t)bufferInfo.size);
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
		...
		vkCmdDraw(commandBuffer, static_cast<uint32_t>(left_vertices.size()), 1, 0, 0);
...
PYBIND11_MODULE(_vulkan, m) {
	...
	m.def("set_window_id", &set_window_id, py::arg("window_id"), "Set the window pointer for Vulkan");

	m.def("set_display_id", &set_display_id, py::arg("display_id"), "Set the windowing system pointer for Vulkan");

	m.def("set_window_system_type", &set_window_system_type, py::arg("backend"), "Set the windowing system type for Vulkan");
	...
	py::class_<VulkanRenderer>(m, "VulkanRenderer")
		...
		.def("setWindowSystemType", &VulkanRenderer::setWindowSystemType, "Tell the renderer what kind of windowing system is being used. This must be done on Linux")
		.def("createSurface", &VulkanRenderer::createSurface, "Create a surface on which to render images");
}
