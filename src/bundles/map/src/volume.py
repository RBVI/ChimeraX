...
class Volume(Model):
  ...
  '''
    ...
    def info_string(self):
        ...
        info = (
            'grid size %d,%d,%d' % tuple(self.data.size) + ', pixel %s' % psize + ', shown at '
        )
    ...
    def replace_data(self, data):
        ...
        d = self.data
        cc = (
            data.origin != d.origin or data.step != d.step or data.cell_angles != d.cell_angles
        )
    ...
    '''
        ...
        for param in ('surface_levels', 'surface_colors', 'image_levels', 'image_colors'):
            if param in kw:
        ...
        if (
            'surface_colors' in kw or 'transparency' in kw or 'image_colors' in kw
            or 'image_brightness_factor' in kw or 'transparency_depth' in kw
        ):
    ...
    def is_full_region(self, region=None, any_step=False):
        ...
        full = (
            tuple(ijk_min) == (0, 0, 0) and tuple(ijk_max) == dmax
    ...
    def initial_image_levels(self, mstats=None, vfrac=(0.01, 0.90), mfrac=None):
        ...
        if polar:
            levels = (
                (mstats.minimum, imax), (max(-vmid, mstats.minimum), imid), (0, ilow),
    ...
    '''
        ...
        if style == 'image' and self.image_shown and not self.surface_shown:
            return
        if (
            style in ('surface', 'mesh') and self.surface_shown
...
#   Doing this to handle switching between orthoplanes, box faces, and volume styles.
    ...
    def first_intercept(self, mxyz1, mxyz2, exclude=None):
        ...
        if self.image_shown:
            ...
            if ro.image_mode == 'full region' and self.single_plane():
                ...
                ijk = tuple(
                    int(round(i)) for i in self.data.xyz_to_ijk(0.5 * (xyz_in + xyz_out))
                )
            ...
            if pd:
                ...
                if d.display_style == d.Mesh or hasattr(pd, 'is_transparent'
    ...
    def bounding_region(self, points, padding=0, step=None, clamp=True, cubify=False):
        ...
        if cubify:
            ijk_min, ijk_max = r[:2]
            s = max(a - b + 1
                    for a, b in zip(ijk_max, ijk_min)) if isinstance(cubify, bool) else cubify
            for a in (0, 1, 2):
                ...
                if sa < s:
                    ...
                    o = (
                        ds + 1
                    ) // 2 if ijk_fmax[a] - ijk_max[a] > ijk_min[a] - ijk_fmin[a] else ds // 2
                    ijk_max[a] += o
                    ijk_min[a] -= ds - o
        if clamp:
    ...
    def matrix_index(self, vijk):
        ...
        mijk = tuple(
            (i - ((m + s - 1) // s) * s) // s for i, m, s in zip(vijk, ijk_min, ijk_step)
        )
        # clamp
    ...
    def matrix_indices_to_xyz_transform(self, step=None, subregion=None):
        ...
        tf = Place(
            (
                (xi - xo, xj - xo, xk - xo, xo), (yi - yo, yj - yo, yk - yo, yo),
    ...
    def interpolated_gradients(
        ...
        gradients, outside = interpolate_volume_gradient(points, v2m_transform, matrix, method)
        if out_of_bounds_list:
    ...
    def copy_zone(self, outside=False):
        ...
        from chimerax.map_data import zone_masked_grid_data
        masked_data = zone_masked_grid_data(self.data, points, radius, invert_mask=outside)
...
class VolumeImage(Image3d):
  ...
  '''
    ...
    def __init__(self, volume):
        ...
        Image3d.__init__(
            self, 'image', v.data, v.region, cmap, v.rendering_options, v.session,
...
class OutlineBox:
    ...
    def _box_outline(self):
        ...
        tlist = (
            (0, 4, 5), (5, 1, 0), (0, 2, 6), (6, 4, 0), (0, 1, 3), (3, 2, 0), (7, 3, 1),
            (1, 5, 7), (7, 6, 2), (2, 3, 7), (7, 5, 4), (4, 6, 7)
        )
    ...
    def _orthoplanes_outline(self):
        ...
        s = self._settings
        corners, center, planes, crosshair_width = s['corners'], s['center'], s['planes'], s[
            'crosshair_width']
        vlist, tlist = [], []
...
class RenderingOptions:
  ...
  '''
    ...
    def __init__(self):
        ...
        self.color_modes = (
            'auto4', 'auto8', 'auto12', 'auto16', 'opaque4', 'opaque8', 'opaque12', 'opaque16',
            'rgba4', 'rgba8', 'rgba12', 'rgba16', 'rgb4', 'rgb8', 'rgb12', 'rgb16', 'la4',
            'la8', 'la12', 'la16', 'l4', 'l8', 'l12', 'l16'
        )
...
def map_from_periodic_map(grid, ijk_min, ijk_max):
    ...
    from chimerax.map_data import ArrayGridData
    g = ArrayGridData(m, xyz_min, grid.step, grid.cell_angles, grid.rotation, name=grid.name)
    return g
...
def set_initial_region_and_style(v):
    ...
    one_plane = (
        getattr(data, 'initial_plane_display', False)
...
class MapChannelsModel(Model, MapChannels):
    ...
    def added_to_session(self, session):
        maps = self.maps
        msg = (
            'Opened multi-channel map %s as #%s, %d channels' %
...
class MultiChannelSeries(Model):
    ...
    def added_to_session(self, session):
        ms = self.map_series
        msg = (
            'Opened multichannel map series %s as #%s, %d channels' %
    ...
    **kw
    ...
    if file_writer(path, format_name) is None:
        ...
        if format_name is None:
            ...
            msg = ('Unknown file format "%s", known formats %s' % (format_name, fmt_names))
        raise UserError(msg)
...
class VolumeUpdateManager:
    ...
    def _display_change(self, tname, m):
        if m.display and m.parents_displayed:
            ...
            vlist = [
                v for v in m.all_models() if v in vset and v.display and v.parents_displayed
            ]
