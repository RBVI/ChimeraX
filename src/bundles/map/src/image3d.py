...
class Image3d(Model):
    ...
    def __init__(
        self, name, grid_data, region, colormap, rendering_options, session, blend_manager
    ):
        ...
        # ImageBlendManager to blend colors with other images
        self._blend_manager = blend_manager
    ...
    def set_region(self, region):
        if (
            region == self._region_unaligned
            and self._data.ijk_to_xyz_transform == self._last_ijk_to_xyz_transform
        ):
            return
        ...
        if bi and self is bi.master_image:
            ...
            # Force redraw since BlendedImage is not in draw hierarchy.
            self.redraw_needed()
    ...
    def _need_color_update(self):
        ...
        if cmode != self._c_mode:
            self._c_mode = cmode
            # Have to change opaque_texture attribute of planes.
            self._remove_planes()
    ...
    def set_options(self, rendering_options):
        ...
        change = False
        for attr in (
            'color_mode', 'colormap_on_gpu', 'colormap_size', 'colormap_extend_left',
            'colormap_extend_right', 'dim_transparent_voxels', 'projection_mode',
            'plane_spacing', 'full_region_on_gpu', 'image_mode', 'linear_interpolation',
            'backing_color'
        ):
        ...
        if (
            rendering_options.image_mode == 'orthoplanes' and ro.image_mode == 'orthoplanes'
            and (
                rendering_options.orthoplane_positions != ro.orthoplane_positions
                or rendering_options.orthoplanes_shown != ro.orthoplanes_shown
            )
        ):
            self._update_planes_for_new_region()

        if rendering_options.maximum_intensity_projection != ro.maximum_intensity_projection:
            # MIP blending is entirely handled in draw routine.
            self.redraw_needed()

        if (
            tuple(rendering_options.tilted_slab_axis) != tuple(ro.tilted_slab_axis)
            or rendering_options.tilted_slab_offset != ro.tilted_slab_offset
            or rendering_options.tilted_slab_spacing != ro.tilted_slab_spacing
    ...
    def _use_3d_texture(self):
        ro = self._rendering_options
        return (
            ro.image_mode == 'tilted slab' or (
                self._p_mode == '3d' and not self._single_plane
    ...
    def _color_array(self, ctype, cshape):
        ...
        try:
            ...
            self.message(
                "Couldn't allocate color array of size (%d,%d,%d,%d) region" % cshape,
                large_data_only=False
            )
            raise
        # TODO: make sure this array is freed.
        self._grayscale_color_array = colors
        return colors
    ...
    def _color_table(self, axis=None):
        ...
        transfer_function_colormap(
            tfa, dmin, dmax, tfcmap, cmap.extend_left, cmap.extend_right
        )
    ...
    def _colormap_properties(self):
        ...
        m = self._c_mode
        if m.endswith('8') or m.endswith('4'):
            t = uint8
        elif m.endswith('16') or m.endswith('12'):
            t = uint16
        else:
            t = uint8
    ...
    def _colormap_components(self):
        ...
        if m.startswith('rgba'):
            c = (0, 1, 2, 3)  # RGBA
        elif m.startswith('rgb'):
            c = (0, 1, 2)  # RGB
        elif m.startswith('la'):
            c = (0, 3)  # RA
        elif m.startswith('l'):
            c = (0,)  # R
        else:
    ...
    def _planes_drawings(self):
        drawings = self._planes_2d_multiaxis + \
            [self._planes_3d, self._planes_2d]
    ...
    def _view_direction(self, render):
        # View direction, scene coords
        return -render.current_view_matrix.inverse().z_axis()
    ...
    def _projection_axis(self, view_direction):
        ...
        ro = self._rendering_options
        if ro.image_mode in (
            'box faces', 'orthoplanes', 'tilted slab'
        ) or self._use_3d_texture:
            return None, False
    ...
    def _make_planes(self, axis, add=True):
    ...
    def draw(self, renderer, draw_pass, plane = None):
        if not self.display:
            return
...
class BackingDrawing(Drawing):
    ...
    def _set_cube_geometry(self):
        ...
        if ro.image_mode == 'tilted slab':
            ...
            tlist = (
                (0, 5, 4), (5, 0, 1), (0, 6, 2), (6, 0, 4), (0, 3, 1), (3, 0, 2), (7, 1, 3),
                (1, 7, 5), (7, 2, 6), (2, 7, 3), (7, 4, 5), (4, 7, 6)
            )
...
class Colormap:
    ...
    def __eq__(self, cmap):
        return (
            self.transfer_function == cmap.transfer_function
            and self.brightness_factor == cmap.brightness_factor
            and self.transparency_thickness == cmap.transparency_thickness
            and self.extend_left == cmap.extend_left and self.extend_right == cmap.extend_right
        )
...
class Texture2dPlanes(PlanesDrawing):
    ...
    def __init__(self, image_render, axis):
        ...
        super().__init__(name, image_render, axis)
    ...
    def _set_axis_planes(self, axis):
        ...
        planes = tuple(
            (k, axis) for k in range(ijk_min[axis], ijk_max[axis] + 1, ijk_step[axis])
        )
    ...
    def _set_box_faces(self):
        ijk_min, ijk_max, ijk_step = self._image_render._region
        planes = (
            tuple((ijk_min[axis], axis)
                  for axis in (0, 1, 2)) + tuple((ijk_max[axis], axis) for axis in (0, 1, 2))
        )
        self._update_geometry(planes)
        return planes
    ...
    def _update_geometry(self, planes):
        ...
        vaa = array(
            (
                ((0, j0, k0), (0, j1, k0), (0, j1, k1),
                 (0, j0, k1)), ((i0, 0, k0), (i1, 0, k0), (i1, 0, k1), (i0, 0, k1)),
                ((i0, j0, 0), (i1, j0, 0), (i1, j1, 0), (i0, j1, 0))
            ), float32
        )
        ...
        ti0, ti1, tj0, tj1, tk0, tk1 = (i0 - fi0 + 0.5) / di, (i1 - fi0 - 0.5) / di, (
            j0 - fj0 + 0.5
        ) / dj, (j1 - fj0 - 0.5) / dj, (k0 - fk0 + 0.5) / dk, (k1 - fk0 - 0.5) / dk
        tca = array(
            (
                ((tj0, tk0), (tj1, tk0), (tj1, tk1),
                 (tj0, tk1)), ((ti0, tk0), (ti1, tk0), (ti1, tk1), (ti0, tk1)),
...
class Texture3dPlanes(PlanesDrawing):
    ...
    def __init__(self, image_render):
        ...
        # Axis, offset, spacing, plane count
        self._last_tilted_slab = (None, None, None, None)
    ...
    def _planes_changed(self, view_direction):
        ...
        if ro.image_mode == 'tilted slab':
            sparams = (
                tuple(ro.tilted_slab_axis), ro.tilted_slab_offset, ro.tilted_slab_spacing,
                ro.tilted_slab_plane_count
            )
    ...
    def _planes_parameters(self, view_direction, scene_position):
        ...
        ro = ir._rendering_options
        if ro.image_mode == 'tilted slab':
            axis, offset, spacing, n = (
                ro.tilted_slab_axis, ro.tilted_slab_offset, ro.tilted_slab_spacing,
                ro.tilted_slab_plane_count
            )
            return self._tilted_slab_plane_parameters(
                view_direction, scene_position, axis, offset, spacing, n
            )
    ...
    def _tilted_slab_plane_parameters(
        self, view_direction, scene_position, axis, offset, spacing, plane_count
    ):
    ...
    def _fill_texture_blend(self, texture):
        ...
        for si in ir.images:
            ...
            if si._use_gpu_colormap:
                b.blend3d(
                    vap.texture, modulation_color, self.texture, vap.colormap,
                    vap.colormap_range
                )
            else:
...
def _box_corners(ijk_region, ijk_to_xyz=None):
    ...
    corners = array(
        (
            (i0, j0, k0), (i1, j0, k0), (i0, j1, k0), (i1, j1, k0), (i0, j0, k1), (i1, j0, k1),
            (i0, j1, k1), (i1, j1, k1)
        ), float32
    )
...
def _xyz_to_texcoord(ijk_region, ijk_to_xyz):
    ...
    v_to_tc = scale((1 / ei, 1 / ej, 1 / ek)) * translation((0.5 - i0, 0.5 - j0, 0.5 - k0)
                                                            ) * ijk_to_xyz.inverse()
    return v_to_tc
...
class BlendedImage(Image3d):
    ...
    def __init__(self, images):
        ...
        self.position = images[
            0].scene_position  # Make sure it has same position as each image.
    ...
    def update_groups(self):
        ...
        for ir in images:
            ...
            if ir.display and ir.parents_displayed and not ro.maximum_intensity_projection:
                orthoplanes = (
                    ro.orthoplanes_shown, tuple(ro.orthoplane_positions)
                ) if ro.image_mode == 'orthoplanes' else None
                rotslab = (
                    tuple(ro.tilted_slab_axis), ro.tilted_slab_offset, ro.tilted_slab_spacing,
                    ro.tilted_slab_plane_count
                ) if ro.image_mode == 'tilted slab' else None
                # Need to have matching grid size, scene position, grid spacing, box face mode, orthoplane mode
                k = (
                    tuple(tuple(ijk)
                          for ijk in ir._region), tuple(ir.scene_position.matrix.flat),
                    tuple(ir._ijk_to_xyz.matrix.flat), ro.image_mode, orthoplanes, rotslab
                )
                if k in aligned:
                    aligned[k].append(ir)
                else:
...
def _step_aligned_region(region):
    ...
    ijk_max_aligned = [
        max(s * (i // s), min) for i, s, min in zip(ijk_max, ijk_step, ijk_min_aligned)
    ]
