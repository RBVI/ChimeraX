...
class DICOMVolume(Volume):
    ...
    def _sphere_grid_bounds(self, center, radius):
        ...
        ijk_max = [
            ...
            for c, s, m in zip(ijk_center, ijk_size, self.data.size)
        ]
    ...
    def segment(self, number):
        ...
        new_seg_model = open_dicom_grids(
            self.session, [new_grid], name="new segmentation"
        )[0]
...
def dicom_volume_from_grid_data(
