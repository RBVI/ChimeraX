...
class PlotDialog:
    ...
    def save_plot(self, *args):
        ...
        smid = SaveMatplotImageDialog(self.session, plot)
        if not smid.exec():
            return
        path = smid.path
        if path is None:
            return
        plot.figure.savefig(path, transparent=smid.transparent_background, dpi=smid.dpi)
