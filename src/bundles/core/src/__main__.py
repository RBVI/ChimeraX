...
localized_app_name = {
    ...
    "en": "ChimeraX",  # English
    "es": "QuimeraX",  # Spanish
    "fi": "KauhukuvaX",  # Finish
    "fr": "ChimèreX",  # French
    "hr": "HimeraX",  # Croatian
    "in": "Angan-anganX",  # Indonesian
    "it": "ChimeraX",  # Italian
    "ja": "キメラX",  # Japanese
    "ko": "키메라X",  # Korean
    "nl": "ChimeraX",  # Dutch
    "no": "ChimeraX",  # Norwegian
    "pl": "ChimeraX",  # Polish
    "pt": "QuimeraX",  # Portuguese
    "ro": "HimerăX",  # Romainian
    "ru": "ХимераX",  # Russian
    "sr": "ХимераX",  # Serbian
    "sk": "PrízrakX",  # Slovak
    "sv": "ChimeraX",  # Swedish
    "th": "ความเพ้อฝันX",  # Thai
    "tr": "KuruntuX",  # Turkish
    "uk": "ХимераX",  # Ukrainian
    "zh": "嵌合體X",  # Chinese
}
        ...
        for opt, optarg in next_arg(argv):
            ...
            if opt == "-c":
                ...
                sys.stdout = io.TextIOWrapper(
                    os.fdopen(sys.stdout.fileno(), "wb"), write_through=True
                )
                sys.stderr = io.TextIOWrapper(
                    os.fdopen(sys.stderr.fileno(), "wb"), write_through=True
                )
            elif opt == "-h":
                opts.help = True
            elif opt == "-d":
...
def parse_arguments(argv):
    ...
    recursive_pip = any(
        [
            argv[1].startswith(os.path.join(path, "pip"))
...
def _set_app_dirs(version):
    ...
    chimerax.app_dirs = ad = appdirs.AppDirs(
        app_name, appauthor=app_author, version=partial_version
    )
    if not is_root:
        ...
        for var, name in (
            ...
            try:
                ...
                print(
                    "Unable to make %s directory: %s: %s"
...
def init(argv, event_loop=True):
    ...
    if (sys.platform.startswith("darwin") and os.getcwd() == "/") or (
        ...
        and (
            os.getcwd().endswith("\\Users\\Public\\Desktop")
            ...
            for cmd in opts.commands:
                try:
                    ...
                    if sess.debug:
                        ...
                        sess.ui.thread_safe(
                            sess.logger.report_exception, exc_info=sys.exc_info()
                        )
    ...
    for arg in args:
        if opts.safe_mode:
            ...
            if not arg.endswith(".py"):
                sess.logger.error(
                    "Can only open Python scripts in safe mode, not '%s'" % arg
                )
                return os.EX_SOFTWARE
