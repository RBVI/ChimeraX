...
def find_nearest(pos, atom, exclude, check_dist, avoid_metal_info=None):
	nearby = search_tree.search_tree(pos.data(), check_dist)
	near_pos = n = near_atom = None
	exclude_pos = set([ex.scene_coord for ex in exclude])
	exclude_pos.add(atom.scene_coord)
    from chimerax.core.geometry import distance
	for nb in nearby:
		n_pos = nb.scene_coord
		if n_pos in exclude_pos:
		...
		for nbb in nb.neighbors:
			if nbb.element.number != 1:
				continue
			if nbb.scene_coord in exclude_pos:
				continue
...
def find_rotamer_nearest(at_pos, idatm_type, atom, neighbor, check_dist):
	...
	near_pos = n = near_atom = None
	for nb in nearby:
		if nb.scene_coord in [at_pos, n_pos]:
...
def new_hydrogen(parent_atom, h_num, total_hydrogens, naming_schema, pos, parent_type_info,
	...
	new_h = add_atom(_h_name(parent_atom, h_num, total_hydrogens, naming_schema),
		Element.get_element(1), parent_atom.residue, pos, serial_number=_serial,
		bonded_to=parent_atom, alt_loc=alt_loc)
	_serial = new_h.serial_number + 1
    from chimera.core.atomic.colors import element_colors
	if new_h.structure in _h_coloring:
		color_scheme = _h_coloring[new_h.structure]
	else:
		parent_color = parent_atom.color
		if parent_color == element_colors(parent_atom.element.number):
...
def _h_name(atom, h_num, total_hydrogens, naming_schema):
	...
	if naming_exceptions.has_key(res_name) and naming_exceptions[res_name].has_key(atom.name):
