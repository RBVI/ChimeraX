...
class SceneThumbnails:
    ...
    def show(self, scenes):
        ...
        for s in scenes:
            ...
            print("Adding memory file: {}".format(mem_name), file=sys.stderr)
            w, h = img.size
        ...
        print("Examining memory file system", file=sys.stderr)
        fs = wx.FileSystem()
        fs.ChangePathTo("memory:")
        name = fs.FindFirst("*")
        while name:
            print("Memory file system name: {}".format(name), file=sys.stderr)
            name = fs.FindNext()
