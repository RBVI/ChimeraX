...
def custom_attrs(self):
    custom_attrs = []
    for attr_name, attr_info in self.__class__._attr_rgeistration.items():
        if hasattr(self, attr_name):
    ...
    def take_snapshot(self, session, flags):
        ...
        return {
            ...
            'registrations': {(bundles[rc].name, rc.__name__):
                rc._attr_registration.take_snapshot(session, flags) for rc in self.registered_classes},
    ...
    def restore_snapshot(session, data):
        ...
        for class_info, registration in data['registrations'].items():
            ...
            if not bundle:
                ...
                continue
            try:
                rc = bundle.get_class(class_name)
                if not rc:
