...
class MapArg(cli.Annotation):
    ...
    def parse(text, session):
        ...
        maps = [m for m in models if isinstance(m, Volume)]
        if len(maps) != 1:
            raise errors.UserError('Must specify one map, got %d' % len(maps))
...
class Int1or3Arg(cli.Annotation):
    ...
    def parse(text, session):
        ...
        try:
            ...
            raise errors.UserError('Must specify integer or 3 comma-separated integers, got %s' % token)
        if len(s) == 1:
            s = (s[0],s[0],s[0])
        if len(s) != 3:
            raise errors.UserError('Must specify integer or 3 comma-separated integers, got %s' % token)
...
class Float1or3Arg(cli.Annotation):
    ...
    def parse(text, session):
        ...
        try:
            ...
            raise errors.UserError('Must specify float or 3 comma-separated floats, got %s' % token)
        if len(s) == 1:
            s = (s[0],s[0],s[0])
        if len(s) != 3:
            raise errors.UserError('Must specify float or 3 comma-separated floats, got %s' % token)
...
class Float2Arg(cli.Annotation):
    ...
    def parse(text, session):
        ...
        try:
            ...
            raise errors.UserError('Must specify 2 comma-separated floats, got %s' % token)
        if len(s) != 2:
            raise errors.UserError('Must specify 2 comma-separated floats, got %s' % token)
        return s, text, rest
...
class MapRegionArg(cli.Annotation):
    ...
    def parse(text, session):
        ...
        try:
            ...
            raise errors.UserError('Must specify 6 comma-separated integers, got %s' % token)
        if len(s) != 6:
            raise errors.UserError('Must specify 6 comma-separated integers, got %s' % token)
        r = s[:3], s[3:]
        return r, text, rest
...
class BoxArg(cli.Annotation):
    ...
    def parse(text, session):
        ...
        try:
            ...
            raise errors.UserError('Must specify 6 comma-separated floats, got %s' % token)
        if len(s) != 6:
            raise errors.UserError('Must specify 6 comma-separated floats, got %s' % token)
        r = s[:3], s[3:]
        return r, text, rest
...
class ValueTypeArg(cli.Annotation):
    ...
    def parse(text, session):
        ...
        if token in types:
            ...
            raise errors.UserError('Unknown data value type "%s", use %s'
...
class IntRangeArg(cli.Annotation):
    ...
    def parse(text, session):
        ...
        n = len(s)
        if n < 2 or n > 3:
            raise errors.UserError('Range argument must be 2 or 3 comma-separateed integer values, got %s' % text)
        i0,i1 = s[:2]
