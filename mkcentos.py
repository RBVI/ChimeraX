...
def main():
    ...
    if error or len(sys.argv) > 3 or os_version not in CENTOS_DEPENDENCIES:
        ...
        print(
            "  Supported CentOS versions are:",
            ", ".join(CENTOS_DEPENDENCIES.keys()),
            file=sys.stderr,
        )
        print(
    ...
    output = subprocess.check_output(
        [CHIMERAX_BIN, "--nocolor", "--version"], stderr=subprocess.DEVNULL
    ).decode()
...
def clean_app():
    ...
    cache_dirs = (
        subprocess.check_output(
            ["/usr/bin/find", CHIMERAX_INSTALL, "-name", "__pycache__"]
        )
        .strip()
        .decode()
        .split("\n")
    ...
    for fn in os.listdir(f"{CHIMERAX_INSTALL}/bin"):
        ...
        if program.startswith(b"/bin/") or program.startswith(b"/usr/bin/"):
            continue
        os.remove(filename)


def make_spec_file(
    rpmbuild_dir, pkg_name, version, rpm_release, bin_path, dependencies
):
    """Make control file"""
